var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
function simpleDecorator(key) {
    console.log("evaluate: ", key);
    return function () {
        console.log("call: ", key);
    };
}
var A = /** @class */ (function () {
    function A(foo) {
    }
    A.method = function (foo) { };
    A.prototype.method = function (foo) { };
    __decorate([
        simpleDecorator('Instance Method'),
        __param(0, simpleDecorator('Instance Method Parameter')),
        __metadata("design:type", Function),
        __metadata("design:paramtypes", [Object]),
        __metadata("design:returntype", void 0)
    ], A.prototype, "method", null);
    __decorate([
        simpleDecorator('Instance Property'),
        __metadata("design:type", Number)
    ], A.prototype, "prop", void 0);
    __decorate([
        simpleDecorator('Static property'),
        __metadata("design:type", Number)
    ], A, "prop", void 0);
    __decorate([
        simpleDecorator('Static Method'),
        __param(0, simpleDecorator('Static Method Parameter')),
        __metadata("design:type", Function),
        __metadata("design:paramtypes", [Object]),
        __metadata("design:returntype", void 0)
    ], A, "method", null);
    A = __decorate([
        simpleDecorator('Class Decorator'),
        __param(0, simpleDecorator('Constructor Parameter')),
        __metadata("design:paramtypes", [Object])
    ], A);
    return A;
}());
