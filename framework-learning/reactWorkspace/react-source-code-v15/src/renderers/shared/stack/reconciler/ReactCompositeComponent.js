...
var ReactCompositeComponent = {
   ...
   * 初始化组件 ReactCompositeComponent[T]，注册事件
  ...
  mountComponent: function(
    ...
    this._context = context; // emptyObject
    this._mountOrder = nextMountID++; // global veriable, accumulative
    this._hostParent = hostParent;
    this._hostContainerInfo = hostContainerInfo;   // ReactDOMContainerInfo

    // { child: ReactElement[1] }
    ...
    // 判断是不是 ReactComponent
    var doConstruct = shouldConstruct(Component);

    // 生成 TopLevelWrapper 的实例
    ...
    // 设置 TopLevelWrapper 实例的值
    inst.props = publicProps;
    inst.context = publicContext;
    inst.refs = emptyObject;
    inst.updater = updateQueue;

    // 重要的链接 ReactCompositeComponent[T] 和 TopLevelWrapper
    this._instance = inst;
  ...
  performInitialMount: function(renderedElement, hostParent, hostContainerInfo, transaction, context) {
    ...
    // 它会调用第一个参数的 mountComponent() 这里是ReactDOMComponent[ins]
    // 现在逻辑已经走到了里层
    // ReactElement[1] 封装进 ReactCompositeComponent[T] 里面，
    // 然后再用它来派生出 ReactDOMComponent[ins]
